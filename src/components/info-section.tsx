...
const InfoSection: FC<{ userData: User; currentWorkspaceData: Workspace; userWorkspaceChannels: Channel[]; currentChannelId: string | undefined }> = ({ userData, currentWorkspaceData, userWorkspaceChannels, currentChannelId }) => {
    ...
    const [isDirectMessageCollapsed, setIsDirectMessageCollapse] = useState(true);
    ...
    return (
        ...
        >
            <div className="w-full flex-col gap-2 p-3">
                ...
                <div>
                    <Collapsible
                        ...
                        onOpenChange={() => setIsDirectMessageCollapse(prevState => !prevState)} 
                        className="flex flex-col gap-2"
                    >
                        <div className="flex items-center justify-between">
                            <CollapsibleTrigger className="flex items-center gap-2">
                                {isDirectMessageCollapsed ? <FaArrowDown /> : <FaArrowUp />}
                                <Typography variant="p" text="Direct Message" className="font-bold" />
                            </CollapsibleTrigger>
                            <div 
                                className={cn(
                                    "cursor-pointer p-2 rounded-full",
            ...
            {/* Render only when currentWorkspaceData is available */}
            {currentWorkspaceData && (
                <CreateChannelDialog 
                    setDialogOpen={setDialogOpen} 
                    dialogOpen={dialogOpen}
                    workspaceId={currentWorkspaceData?.id}  // Safe access to workspace id
                    userId={userData.id}  // Assuming userData is always present
                />
            )}
        </div>
    );
